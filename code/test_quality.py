...
# put this code to folder :
# D:\LYIT\repository\No-Reference-Image-Quality-Assessment-using-BRISQUE-Model\Python\libsvm\python\
# execute this command line
# python D:\LYIT\repository\No-Reference-Image-Quality-Assessment-using-BRISQUE-Model\Python\libsvm\python\test_quality.py  D:\\LYIT\\dissertation\\poor_quality\\
# 
...
def test_measure_BRISQUE(imgPath):
    ...
    # load model 
    model = svmutil.svm_load_model("D:\\LYIT\\repository\\No-Reference-Image-Quality-Assessment-using-BRISQUE-Model\\Python\\allmodel")
...
if(len(sys.argv) != 2):
    ...
    sys.exit(0)
print('load images from folder ' + sys.argv[1])
list = os.listdir(sys.argv[1])
file = ''
n = 1
for i in range(0,len(list)-1) :
    file = sys.argv[1] + list[i]
    # calculate quality score
    qualityscore = test_measure_BRISQUE(file)
    print("." * i)
    if(qualityscore < 30):
        n += 1
        print ("Score of the given image {}\t{}".format(file, qualityscore)) 
print ("{} of {} images score is lower than 20".format(n, len(list)))